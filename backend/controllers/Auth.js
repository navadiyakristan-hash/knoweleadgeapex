...
exports.signup = async (req, res) => {
    try {
        console.log('req.body:', req.body); // Debugging incoming data
        ...
        console.log('User created:', createdUser); // Debugging created user
...
exports.login = async (req, res) => {
    try {
      ...
      const COOKIE_EXPIRATION_DAYS = 7; // Set the cookie expiration to 7 days
      const PRODUCTION = false; // Change to true if running in production with HTTPS
      ...
      if (existingUser && (await bcrypt.compare(req.body.password, existingUser.password))) {
        ...
        const maxAge = COOKIE_EXPIRATION_DAYS * 24 * 60 * 60 * 1000; // Convert to milliseconds
        console.log("Computed maxAge for cookie:", maxAge);
  
        // sending jwt token in the response cookies
        res.cookie('token', token, {
          sameSite: PRODUCTION ? 'None' : 'Lax',
          maxAge: maxAge, // Valid number
          httpOnly: true,
...
exports.verifyOtp=async(req,res)=>{
    try {
        ...
        if(!isValidUserId){
            return res.status(404).json({message:'User not Foundssssss, for which the otp has been generated'})
        }
